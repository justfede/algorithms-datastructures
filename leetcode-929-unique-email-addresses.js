/*
    ...
    Solution Big-O: 
        Time Complexity: Scan is linear but i'm not sure about the complexity of split() and replace(). Isn't a good solution x_x
        
*/  

let input =  ["test.email+alex@leetcode.com","test.e.mail+bob.cathy@leetcode.com","testemail+david@lee.tcode.com"];

const numUniqueEmails = (input) => {
    var cleanEmails = new Map();
    for (var i = 0; i < input.length ; i ++){
        var [local, domain] = input[i].split('@');
        local = local.split("+")[0].replace(/\./g,'');
        cleanEmails.set(local+ "@" + domain);
    }
